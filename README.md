...
This folder (layer) is where we add everything related to external "**Frameworks and Drivers**". Here you can add the repositorys, external frameworks, web and so on.
...
This layer is where we put everything together. It's the "dirty layer", because most factorys will be here. It may depend on all other layers in the system.
...
In our case, data enter the system via "express routes" (infrastructure layer) to the "controllers" (adapters layer), to the "use cases" (application layer), to the "domain" (domain/entities layer). Then the system outputs via "domain" -> "use cases" -> "presentation" -> "infrastructure". I'm using the "main" layer to create factorys for "middlewares", and "controllers" (and possibly other factorys).
...
│   │   └── repositorys
│   │       ├── brand-repository-mongodb.ts
│   │       ├── category-repository-mongodb.ts
│   │       ├── product-repository-mongodb.ts
